...
def max_heapify(A,i):
    ...
    if (l <= A.heap_size-1) and (A[l].priority > A[i].priority):
      largest = l 
    else:
      largest = i 
    
    if (r <= A.heap_size-1) and (A[r].priority > A[largest].priority):
      largest = r 
...
def max_heap_insert(A,task):
  A.heap_size += 1
  A.append(task)
  A[len(A)-1],A[A.heap_size-1] = A[A.heap_size-1], A[len(A)-1]
